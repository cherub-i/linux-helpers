...
MAIL_SENDER=${MAIL_SENDER:-'sender@mail.com'}
MAIL_RECIPIENT=${MAIL_RECIPIENT:-'recipient@mail.com'}
...
if [ ${problems} -eq 0 ]; then
  ...
  for volume in ${zfsVolumes}
   do
    ...
    # https://gist.github.com/geggo98/841773ab1f3360c67d21#gistcomment-1798438 > Hi, there's a small problem in this script, such that if the last operation was a completed resilver, the scrub agecheck is never performed:
    #   scan: resilvered <size> in <time> with 0 errors on Sat May 28 11:38:17 2016


    ### Ubuntu with GNU supported date format
    scrubRawDate=$(/sbin/zpool status $volume | grep scrub | awk '{print $(NF-3)" "$(NF-2)" "$NF" "$(NF-1)}')
...
if [ "$problems" -ne 0 ]; then
  printf '%s\n' "$emailSubject" "" "`/sbin/zpool list`" "" "`/sbin/zpool status`" | /usr/bin/mail -s "$emailSubject" -r $MAIL_SENDER $MAIL_RECIPIENT
  logger $emailSubject
fi
